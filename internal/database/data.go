...
// CreateStoredData creates new stored data in the database.
...
// GetStoredDataByID retrieves stored data by ID.
...
// GetStoredDataByUserID retrieves all stored data for a user.
...
// GetStoredDataByUserIDSince retrieves stored data for a user since a specific time.
...
// UpdateStoredData updates existing stored data.
...
// DeleteStoredData marks stored data as deleted by ID.
...
// SyncStoredData synchronizes stored data for a user.
func (db *DB) SyncStoredData(userID string, dataList []models.StoredData, lastSyncAt time.Time) ([]models.StoredData, error) {
	...
	for _, clientData := range dataList {
		...
		if existingData == nil {
			...
			// Update existing data if client version is newer
